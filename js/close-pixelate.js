...
(function(window, undefined) {
  ...
  function ClosePixelation(img, options) {
    ...
    this.ctx.filter = "contrast(200) saturate(150) brightness(50)";

    this.render(options)
    // replace image with canvas
    // img.parentNode.replaceChild(canvas, img)
    // let index = img.parentNode.children.indexOf(img)
    let children = img.parentNode.children;
    let list = []
    for(let i=0;i<children.length;i++){
      list.push(children[i])
    }
    img.parentNode.insertBefore(canvas, list[list.indexOf(img)+1])
    // img.parentNode.insert
